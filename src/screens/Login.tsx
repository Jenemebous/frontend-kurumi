  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: 'hsl(0, 0%, 5%)' }} className="flex items-center justify-center">
      <form onSubmit={handleSubmit} className="w-full max-w-md p-6 rounded-lg shadow-md" style={{ backgroundColor: '#2c2c2c', color: 'whitesmoke' }}>
        ...
        />
        <input
          className="w-full p-2 mb-6 rounded bg-gray-800 border border-gray-600 placeholder-gray-400"
          type="password"
          placeholder="Senha"
          value={password}
          ...
          required
        />
        <button type="submit" className="w-full bg-red-800 hover:bg-red-700 text-white py-2 rounded">Login</button>
        <p className="text-sm text-center mt-4">
          Não tem uma conta? <Link to="/register" className="text-red-400 hover:underline">Cadastre-se</Link>
        </p>
      </form>
    </div>
  );
